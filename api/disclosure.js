export default async function handler(req, res) {
  ...
  try {
    if (type === 'sec') {
      ...
      // DART API로 한글 회사명 조회
      const apiKey = process.env.DART_API_KEY || '';
      if (!apiKey) return res.status(200).json({ corp_name: '' });
      const code = stockCode || symbol || '';
      const url = `https://opendart.fss.or.kr/api/company.json?crtfc_key=${apiKey}&stock_code=${code}`;
      const r = await fetch(url);
      const data = await r.json();
      res.status(200).json({ corp_name: data.corp_name || '', stock_code: code });

    } else {
      res.status(400).json({ error: 'type required: sec | dart | corpname' });
    }
  } catch (e) {
